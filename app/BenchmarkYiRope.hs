...
groups :: [Benchmark]
groups = [textBenchmarks]

textBenchmarks :: Benchmark
textBenchmarks =
  let initial =
        force $ moveCursor (initRow, initCol) $ textZipper content Nothing
      h       = 50000 -- in lines
      w       = 10 -- in words
      ws      = ["foo", "bar", "things", "stuff"]
      content = replicate h line
      line    = Y.concat $ intersperse " " $ take w $ concat $ repeat ws
      initRow = h `div` 2
      initCol = (Y.length line) `div` 2
  in  bgroup
        "text"
